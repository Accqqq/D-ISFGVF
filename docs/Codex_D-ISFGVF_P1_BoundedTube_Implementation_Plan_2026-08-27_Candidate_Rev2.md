# D-ISFGVF P1 Bounded Nominal Asymmetric Tube — Candidate Rev2

~~~text
DOCUMENT_ROLE=P1_IMPLEMENTATION_PLAN
DOCUMENT_REVISION=REV2
DOCUMENT_STATUS=CANDIDATE / READY FOR SOL XHIGH FINAL RE-REVIEW
P1_SCOPE=BOUNDED_NOMINAL_ASYMMETRIC_TUBE
SUPERSEDES_CANDIDATE_PATH=docs/Codex_D-ISFGVF_P1_BoundedTube_Implementation_Plan_2026-08-27_Candidate_Rev1.md
SUPERSEDES_CANDIDATE_SHA256=047c571241915aaa09da5e751a72e4c026ca209c591f0fc1aec6b3772929a55e
...
SOL_P1_PLAN_AUDIT=PENDING_REV2_REVIEW
P1_IMPLEMENTATION_AUTHORIZED=false
LUNA_AUTHORIZED=false
~~~

This is the complete self-contained Rev2 P1 Candidate Plan. It supersedes the
Rev1 Candidate for review but does not modify it. It contains only the
documentation corrections requested by the second fresh Sol xhigh audit.
...
4. this exact Candidate Rev2 after the final fresh Sol acceptance;
...
~~~text
total_tube_construction_queries =
  cross_section_queries + builder_path_cell_queries
  + validator_surface_queries
...
These classes are never combined into one ambiguous maximum.
...
|---|---:|---:|---:|
| complete matching cell certificate | 0 | midpoint position variation + 0.5*max_delta*normal variation + 0.5*delta_slope*h + 0.5*snapshot_resolution | planner_safe_distance + C + cover_epsilon |
...
candidate. TubeSurfaceValidator owns the single geometric cover. The
half-voxel support is charged once except for the proven exact-anchor
degenerate branch, and planner_safe_distance is charged once. A certificate
failure never silently falls back to sampled cover.
...
31. the Contract-A residual table is preserved term-for-term, including the
    exact-current-anchor branch and the no-fallback proof-failure result;
...
34. the metric-routing accounting identity and observable collectors below
    are satisfied.
...
|---|---|---|---|---|---|
| bounded-construction max, Builder/Validator/inward/query/max-offset diagnostics | TubeBuilder / CertifiedTubeBuilder / Validator-facing P1 seams | `TubeBuildDiagnostics` fields written by the builder | CertifiedTubeBuilder profile diagnostics -> TubeEpochManager candidate/result -> adapter | existing tube diagnostics/measurement route | per build; maxima and counts over the complete run |
...
~~~text
total_tube_construction_queries =
  cross_section_queries + builder_path_cell_queries + validator_surface_queries
total_current_safety_queries =
  retained_reference + actual_position + base_centerline
~~~

Builder/Validator/inward/query/max-offset diagnostics are not silently
reinterpreted as current-safety counts. `cmdCallback` reuses the unchanged
...
Rev2 is the only new file in this documentation correction round. Existing
...
After Rev2, compute its exact SHA and byte size; verify branch, HEAD, remotes,
status, staged paths zero, and git diff --check. Do not stage or commit.

If the final Sol requires further documentation corrections, stop without a
Rev3. At most two correction/review rounds after the initial Candidate are
authorized; Rev2 is the last permitted correction round. A blocker requiring
code or architecture expansion stops the workflow.

## 23. Sol xhigh Rev2 review checklist and output

A new independent Sol xhigh reviewer must verify the exact Rev2 artifact
identity, closed
...
Every MUST_FIX must cite severity, exact Rev2 section, source file/function
...
This Rev2 Candidate does not authorize production or test implementation,
...
fresh Sol xhigh read-only audit of this exact Rev2 hash. If accepted, a
